...
function renderDraw() {
  ...
  qa(".skipMirror").forEach((x) => x.textContent = S.skips);
  q("#changeTeam").disabled = S.skips <= 0;
}
...
function openPlayers() {
  ...
  q("#playerGrid").innerHTML = pool.map((p) => `<button class="player-card" data-player="${p.id}"><div class="portrait"><b>${ini(p.name)}</b>${S.difficulty === "hard" ? "" : `<span class="ovr">${p.overall}</span>`}</div><div class="player-info"><b>${p.name}</b><small>${p.role} · ${p.team} · Split 2</small></div></button>`).join("");
...
function startLiveMatch(opp, oppStrength, mode, label) {
  S.live = { opp, os: oppStrength, mode, label, m: 0, h: 0, a: 0, paused: false, done: false, timer: null, goals: [], shots: [0, 0], saves: [0, 0] };
