...
function bind() {
  ...
  q("#spinPlayer").onclick = spinPlayers;
  q("#freeSpin").onclick = freeSpinPlayers;
...
function draftRespinCount() {
  return S.difficulty === "easy" ? 2 : 1;
}

function spinPlayerCount() {
...
function renderPitch() {
  ...
  q("#pitch").innerHTML = S.slots.map((s, i) => {
    ...
    return `<div class="slot ${p ? "filled" : ""}" style="left:${x}%;top:${y}%">${p
      ? `<span class="ini">${ini(p.name)}</span><span class="info">${p.name}<br>${p.role} · ${S.difficulty === "hard" ? "?" : p.overall}</span>`
      : `<span><b class="plus">+</b><br><b class="role">${s.role}</b></span>`}</div>`;
  }).join("");
  const filled = S.slots.filter((x) => x.player).length;
  const remaining = S.slots.length - filled;
  q("#draftStatus").textContent = remaining === S.slots.length
    ? "Premi Spin Player per iniziare."
    : `Rosa: ${filled}/${S.slots.length} · Mancano ${remaining} giocator${remaining === 1 ? "e" : "i"}.`;
  q("#spinPlayer").textContent = filled ? "🎲 SPIN PLAYER" : "🎲 PRIMO SPIN";
}

function openRoles() {
  return [...new Set(S.slots.filter((s) => !s.player).map((s) => s.role))];
}

function remainingRoleCounts() {
  const counts = {};
  S.slots.filter((s) => !s.player).forEach((s) => counts[s.role] = (counts[s.role] || 0) + 1);
  return counts;
}

function availableAny() {
  return PLAYERS.filter((p) => !S.used.has(p.id));
}

function hasOpenSlot(role) {
  return S.slots.some((s) => !s.player && s.role === role);
}

function lastMissingRole() {
  const empty = S.slots.filter((s) => !s.player);
  return empty.length === 1 ? empty[0].role : null;
}

function buildRandomSpin() {
  const pool = availableAny();
  const limit = Math.min(spinPlayerCount(), pool.length);
  S.shownPlayers = shuffle(pool).slice(0, limit);
  S.forcedFinalRole = lastMissingRole();
}

function spinPlayers() {
  if (S.slots.every((x) => x.player)) return summary();
  buildRandomSpin();
  renderPlayers();
  show("players");
}

function freeSpinPlayers() {
  if (!S.forcedFinalRole || S.skips > 0) return;
  buildRandomSpin();
  renderPlayers();
}

function renderPlayers() {
  const finalRole = lastMissingRole();
  S.forcedFinalRole = finalRole;
  const validShown = S.shownPlayers.filter((p) => hasOpenSlot(p.role));
  const finalRoleShown = finalRole ? S.shownPlayers.filter((p) => p.role === finalRole) : [];
  const noUsablePlayer = validShown.length === 0;
  const mustForceRespin = ((!!finalRole && finalRoleShown.length === 0) || noUsablePlayer) && S.skips > 0;
  const freeSpinAvailable = ((!!finalRole && finalRoleShown.length === 0) || noUsablePlayer) && S.skips <= 0;

  q("#playersCount").textContent = `${S.shownPlayers.length} giocator${S.shownPlayers.length === 1 ? "e" : "i"} estratti · Ruoli liberi: ${openRoles().join(", ")}`;
  q("#playerGrid").innerHTML = S.shownPlayers.map((p) => {
    const validRole = hasOpenSlot(p.role);
    const forcedOut = finalRole && p.role !== finalRole;
    const selectable = validRole && !forcedOut && !mustForceRespin && !freeSpinAvailable;
    const overall = S.difficulty === "hard" ? "?" : p.overall;
    let reason = "";
    if (!validRole) reason = "Ruolo già completo";
    else if (forcedOut) reason = `Serve ${finalRole}`;
    else if (mustForceRespin) reason = "Usa il Respin obbligatorio";
    else if (freeSpinAvailable) reason = "Fai uno Spin gratis";
    return `<button class="player-card ${selectable ? "" : "disabled-card"}" data-player="${p.id}" ${selectable ? "" : "disabled"}>
      <div class="portrait"><b>${ini(p.name)}</b><span class="ovr">${overall}</span></div>
      <div class="player-info"><b>${p.name}</b><small>${p.role} · ${p.team} · Split 2</small>${reason ? `<em>${reason}</em>` : ""}</div>
    </button>`;
  }).join("");
  ...
  respin.classList.toggle("forced-action", mustForceRespin);
  q("#freeSpin").classList.toggle("hidden", !freeSpinAvailable);

  const notice = q("#spinNotice");
  if (mustForceRespin) {
    notice.innerHTML = finalRole
      ? `⚠️ Ti manca solo <b>${finalRole}</b> e non è uscito. Hai ancora un Respin: devi usarlo.`
      : `⚠️ Nessun giocatore estratto può occupare i posti rimasti. Hai ancora un Respin: devi usarlo.`;
    notice.classList.remove("hidden");
  } else if (freeSpinAvailable) {
    notice.innerHTML = finalRole
      ? `🎁 Ti manca solo <b>${finalRole}</b>. Non hai più Respin: usa Spin gratis finché compare il ruolo richiesto.`
      : `🎁 Nessun giocatore estratto è utilizzabile. Non hai più Respin: usa Spin gratis per continuare.`;
    notice.classList.remove("hidden");
  } else if (finalRole) {
    notice.innerHTML = `🎯 Ultimo posto: puoi scegliere solo un giocatore <b>${finalRole}</b>.`;
    notice.classList.remove("hidden");
  } else if (!validShown.length) {
    notice.innerHTML = "Nessuno dei ruoli estratti ha ancora un posto libero. Usa il Respin, se disponibile.";
    notice.classList.remove("hidden");
  } else {
...
function respinPlayers() {
  ...
  S.skips--;
  renderPlayers();
  addToast(`Respin utilizzato: ne rimangono ${S.skips}.`);
}

function pick(id) {
  const p = PLAYERS.find((x) => String(x.id) === String(id));
  if (!p || !hasOpenSlot(p.role)) return;
  const finalRole = lastMissingRole();
  if (finalRole && p.role !== finalRole) return;
  const target = S.slots.find((s) => !s.player && s.role === p.role);
  if (!target) return;
  target.player = p;
  S.used.add(p.id);
  S.shownPlayers = [];
